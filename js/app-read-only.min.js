...
</div>`}$("#content").html(b)})}function file_code(a){var c={html:"html",php:"php",css:"css",go:"golang",java:"java",js:"javascript",json:"json",txt:"Text",sh:"sh",md:"Markdown"},b=a.split("/").pop();unescape(b);var d=b.split(".").pop().toLowerCase(),a=a;UI.second_domain_for_dl?UI.downloaddomain:window.location.origin,$.post("",function(c){try{var a=jQuery.parseJSON(gdidecode(read(c))),d=formatFileSize(a.size),b=`
...
</div>`}$("#content").html(b)}),$.get(a,function(a){$("#editor").html($("<div/><div/><div/>").text(a).html()),void 0!=c[d]&&c[d]})}function file_video(a){var b=a.split("/").pop();unescape(b),b.slice(0,b.lastIndexOf("."));var a=a,c=UI.second_domain_for_dl?UI.videodomain+a:window.location.origin+a;c.replace(new RegExp("\\[","g"),"%5B").replace(new RegExp("\\]","g"),"%5D").replace(new RegExp("\\!","g"),"%25"),c.replace(/^(https?:|)\/\//,""),btoa(c),$.post("",function(c){try{var a=jQuery.parseJSON(gdidecode(read(c))),d=formatFileSize(a.size);null!=a.thumbnailLink?a.thumbnailLink.slice(0,-5):UI.poster;var b=`
...
</div>`}$("#content").html(b)})}function file_audio(a){unescape(a.split("/").pop());var a=a,b=UI.second_domain_for_dl?UI.downloaddomain+a:window.location.origin+a;$.post("",function(d){try{var a=jQuery.parseJSON(gdidecode(read(d))),e=formatFileSize(a.size),c=`
  <div class="container"><br>
...
</div>`}$("#content").html(c)})}function file_pdf(a){unescape(a.split("/").pop());var a=a;UI.second_domain_for_dl?UI.downloaddomain:window.location.origin,$.post("",function(c){try{var a=jQuery.parseJSON(gdidecode(read(c))),d=formatFileSize(a.size),b=`
  <script>
