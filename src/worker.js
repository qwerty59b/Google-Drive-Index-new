	...
	if (environment === 'production') {
		app_js_file = uiConfig.jsdelivr_cdn_src + '@' + uiConfig.version + '/js/app.obf.js'
	...
	async function handleRequest(request, event) {
		...
		if (match) {
			...
			if (command === 'search') {
				...
				if (type && type == 'folder') {
					...
					for (const file of details.data.files) {
						if (file.mimeType != 'application/vnd.google-apps.folder') {
							file.link = await generateLink(file.id);
						}
				...
				details.link = await generateLink(details.id);
